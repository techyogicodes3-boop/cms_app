...
export default function AccountPage() {
  ...
  useEffect(() => {
    ...
    api.get('/api/v1/user/dashboard')
      ...
      .catch((error) => toast.error(error?.response?.data?.message || 'Could not load your account.'))
  ...
  const updateAddress = (field, value) => setProfile((current) => ({
    ...current,
  ...
  const saveProfile = async (event) => {
    ...
    try {
      ...
      localStorage.setItem('user', JSON.stringify({ ...stored, name: saved.name }));
          ...
          <div className="grid gap-6 lg:grid-cols-[0.85fr_1.35fr]">
            <section className="rounded-lg border border-[#E8D8CC] bg-[#FFFCF8] p-5 shadow-[0_10px_30px_rgba(43,20,14,0.07)] sm:p-6">
              <div className="flex items-center justify-between gap-3">
                ...
                {!editing && <button type="button" onClick={() => setEditing(true)} className="inline-flex items-center gap-2 rounded-lg border border-[#C98A78] px-3 py-2 text-sm font-bold hover:bg-[#F6ECDD]"><Edit3 className="h-4 w-4" /> Edit</button>}
              </div>

              <form onSubmit={saveProfile} className="mt-5 space-y-4">
                <label className="block text-sm font-semibold">Full name<input required disabled={!editing} value={profile.name} onChange={(event) => update('name', event.target.value)} className="mt-1.5 h-11 w-full rounded-lg border border-[#E8D8CC] bg-[#FFF9F3] px-3 disabled:opacity-70" /></label>
                <label className="block text-sm font-semibold">Email<input disabled value={profile.email} className="mt-1.5 h-11 w-full rounded-lg border border-[#E8D8CC] bg-[#F6ECDD] px-3 opacity-70" /></label>
                <label className="block text-sm font-semibold">Phone<input disabled={!editing} value={profile.phone} onChange={(event) => update('phone', event.target.value)} className="mt-1.5 h-11 w-full rounded-lg border border-[#E8D8CC] bg-[#FFF9F3] px-3 disabled:opacity-70" /></label>
                <div className="flex items-center gap-2 pt-2 text-sm font-bold"><MapPin className="h-4 w-4 text-[#D85C6B]" /> Default address</div>
                <label className="block text-sm font-semibold">Street address<input disabled={!editing} value={profile.address.streetAddress} onChange={(event) => updateAddress('streetAddress', event.target.value)} className="mt-1.5 h-11 w-full rounded-lg border border-[#E8D8CC] bg-[#FFF9F3] px-3 disabled:opacity-70" /></label>
                <div className="grid gap-3 sm:grid-cols-2">
                  {['city', 'state', 'zipcode'].map((field) => <label key={field} className="block text-sm font-semibold capitalize">{field === 'zipcode' ? 'ZIP code' : field}<input disabled={!editing} value={profile.address[field]} onChange={(event) => updateAddress(field, event.target.value)} className="mt-1.5 h-11 w-full rounded-lg border border-[#E8D8CC] bg-[#FFF9F3] px-3 disabled:opacity-70" /></label>)}
                </div>
                {editing && <div className="flex gap-2 pt-2"><button disabled={saving} className="inline-flex h-11 items-center gap-2 rounded-lg bg-[#D85C6B] px-4 text-sm font-bold text-white disabled:opacity-60"><Save className="h-4 w-4" /> {saving ? 'Saving…' : 'Save profile'}</button><button type="button" onClick={() => { setProfile(data.profile); setEditing(false); }} className="h-11 rounded-lg border border-[#E8D8CC] px-4 text-sm font-bold">Cancel</button></div>}
              </form>
            </section>
